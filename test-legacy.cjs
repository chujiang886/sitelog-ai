...
(async()=>{
 ...
 const context=await browser.newContext({viewport:{width:1440,height:1000},acceptDownloads:true}),page=await context.newPage();page.on('dialog',d=>d.accept());const errors=[];page.on('pageerror',e=>errors.push({message:e.message,stack:e.stack,phase}));await login(page);
 ...
 for(let index=0;index<ids.length;index++){
  ...
  phase="复制历史";await page.evaluate(id=>Alpine.$data(document.body).copyLegacyProject(id),id);
  ...
  phase="改稿保存";const first=page.locator('#images-container .node-card-desc').first();const manual=(await first.innerText())+'\n隔离测试补记';await first.fill(manual);
  ...
  phase="关闭分享弹窗";await page.evaluate(()=>{const a=Alpine.$data(document.body);a.shareDialogOpen=false;a.projectPanel=false;window.trialPrintHtml='';const append=document.body.appendChild.bind(document.body);document.body.appendChild=function(n){const result=append(n);if(n.tagName==='IFRAME')n.contentWindow.print=()=>{window.trialPrintHtml=n.contentDocument.documentElement.outerHTML};return result}});
  phase="导出PDF";if(process.env.TEST_PDF==='1'){
    await page.getByRole('button',{name:'导出 PDF',exact:false}).first().click();await page.getByRole('button',{name:'⬇️ 下载 PDF（文字可改）',exact:true}).click();await page.waitForFunction(()=>window.trialPrintHtml?.length>0);const html=await page.evaluate(()=>window.trialPrintHtml);assert.ok(html.includes('隔离测试补记'));
  ...
  phase="撤回分享";await page.evaluate(async sid=>Alpine.$data(document.body).accountJSON('/publication/'+sid+'/revoke',{}),share.id);await view.reload();await view.getByText('分享链接已失效',{exact:true}).waitFor();
