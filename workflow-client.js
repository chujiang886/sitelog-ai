...
(() => {
 ...
 window.workflowFeatures={
  ...
  async loadProjects(){const owner=this.accountUser?.id;const params=new URLSearchParams({q:this.projectQuery,page:this.projectPage,stage:this.archiveStage,person:this.archivePersonFilter,after:this.archiveAfter,before:this.archiveBefore,kind:this.archiveKind});const data=await this.accountJSON('/archives?'+params);if(this.accountUser?.id!==owner)return;this.projectItems=data.items;this.projectTotal=data.total},
  async previewArchive(item){const owner=this.accountUser?.id;this.archiveGallery=[];this.archiveGalleryOpen=true;await this.projectAction(async()=>{if(item.kind==='legacy')throw Error('历史档案可通过原分享查看，复制后可使用云端缩略图');const data=await this.accountJSON('/projects/'+item.id);if(this.accountUser?.id!==owner)return;this.archiveGallery=sitelogEditor.collections.flatMap(g=>data.body[g]||[]).map(p=>({id:p.id,title:p.title,src:'/api/share/media/'+p.mediaId+'/thumbnail'}))})},
  ...
  async openFeedback(){const owner=this.accountUser?.id,pid=this.cloudProjectId;this.feedbackPanel=true;await this.projectAction(async()=>{if(!pid)throw Error('请先保存云端工程');const data=await this.accountJSON('/projects/'+pid+'/feedback');if(this.accountUser?.id===owner&&this.cloudProjectId===pid)this.feedbackItems=data.items})},
