"""型材（Profile）计算单元（Sprint G 任务1 + 任务4）。

设计依据：Sprint F 型材设计文档（已审核）。本文件承载 profile 接口
的计算结构装配，并定义结果模型 ``ProfileResult``。

红线（Sprint G，强制）：
- 本模块**不计算**任何真实型材数值；D-TH-01 在 verified.json 中仍为
...
- 不填真实壁厚（t）、截面惯性矩（I）、截面模量（W）、强度设计值（f）、弹性模量（E）；
- 不写规范条款号，不写死工程常数；
- 所有未知一律 pending_verification。

跨模块链路（任务4）：消费上游 wind_pressure 的 w_k；若 wind_pressure 的
verification_status 不等于 engineering_approved（或 w_k 不可得），profile
...
# 型材接口标识（与 INTERFACE_THRESHOLD_MAP 对齐；阈值数值 pending_verification）。
PROFILE_INTERFACE: str = "profile"


@dataclass
class ProfileResult(EngineeringCalculationResult):
    """profile 计算单元的统一结果模型（Sprint 3.2.1 继承自基类）。

    九字段与 as_interface() / as_full() 由 ``EngineeringCalculationResult``
    统一提供；本类仅声明接口标识 INTERFACE。

    红线：result 在 pending 态为空串（不产出真实型材数值）；
...
class ProfileCalculator:
    ...
    - ``project``：项目几何 / 支承条件 / 杆件跨度（多为 inferred）；
    - ``design_candidate``：Design Agent 候选（frame_series / frame_material /
      dimensions_hint / field_provenance）；
    - ``wind_pressure_result``：WindPressureResult 产出（上游 w_k 供给方）；
    - ``threshold_refs``（可选）：外部传入引用，缺省从阈值库加载。

    红线：本计算器**不产出**任何真实型材数值，所有数值位保持 pending 占位；
    ...
    def _read_provenance(
        ...
        # 设计候选关键字段溯源（型材系列 / 材质，pending_verification）。
        for key in ("frame_series", "frame_material"):
            provenance[f"design.{key}"] = str(des_prov.get(key, "inferred"))
        # 项目几何字段（杆件跨度 / 支承条件 / 建筑高度）来自 Vision/Design 候选 → inferred。
        for key in ("member_span", "support_condition", "building_height"):
    ...
    def _is_wind_pressure_approved(
        ...
        中 w_k.value 非 None 才视为可信；否则 profile 必须 pending。
        """
    ...
    def calculate(self, context_data: Mapping[str, Any]) -> ProfileResult:
        """执行 profile 计算结构的装配（不产出真实数值）。

        返回 ``ProfileResult``：``verification_status`` 恒
        ...
        intermediate: dict[str, Any] = {
            ...
            "q": {
                "value": None,
                "unit": "N/mm",
                "source": "derived (w_k * geometry)",
                "verified": False,
            },
            "M": {
                "value": None,
                "unit": "N·mm",
                "source": "derived (q * L * support)",
                "verified": False,
            },
            "N": {
                "value": None,
            ...
            "sigma": {
                "value": None,
                "unit": "MPa",
                "source": "derived (M / W + N / A)",
                "verified": False,
            },
            "delta": {
                "value": None,
                "unit": "mm",
                "source": "derived (q * L * E * I * support)",
                "verified": False,
            },
            "delta_lim": {
                "value": None,
                "unit": "mm",
                "source": "project requirement / norm",
                "verified": False,
            },
        }
        ...
        formula: str = "; ".join(profile_rules.PROFILE_FORMULAS)
        evidence: str = (
            f"公式来源：型材采用荷载—内力—应力/挠度校核结构 {formula}"
            "（系数选取规则 pending_verification）。"
            "变量来源：型材壁厚 t、截面惯性矩 I、截面模量 W、截面积 A、强度设计值 f、"
            "弹性模量 E 来自工程阈值库 D-TH-01；"
            "风荷载标准值 w_k 来自上游 wind_pressure；上述阈值均未双签，"
            "取值 pending_verification。"
            "上游 w_k 不可信时不得基于未验证风压推出型材达标结论（跨模块降级）。"
            "D-TH-01 未双签前不得产出具体壁厚/截面属性/强度/挠度数值（红线）。"
        )
