...
- **后端 API**（已交付 `backend/app/api/governance_activation_simulation.py`，6 端点）：`GET /`、`/scenarios`、`/negative-paths`、`/report/latest`、`/report/{id}`，`POST /run`；全部 `csrf_protect` + `RELEASE_READ`，**无 `/activate` / `/deploy-production` / `/go` 端点**。
