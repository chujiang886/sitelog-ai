#!/usr/bin/env python3
"""Phase 3.9.14 Branch Integrity Gate.

Fail-closed CI 守卫：在 commit / CI 前核验当前工作树未偏离 Phase 3.9.14
External Staging Runtime Deployment & End-to-End Qualification 的合法边界。

检查项：
  1. 当前分支必须 = feat/phase3.9.14-external-staging-runtime-deployment-e2e-qualification
  2. 工作树（tracked + untracked）不得出现 forbidden 模块名：
     production_handoff / handoff
  3. 不得出现下一 Phase 编号泄漏（3.9.15）
  4. 旧 WIP carryover：agents/enterprise/production_handoff/ 不得出现
  5. Audit Ledger total 必须 = 129（本阶段 0 新增企业类目；3.9.14 执行审计类别为
     自包含常量集，不污染企业 AuditActionCategory 枚举与冻结账本）

设计约束（规避沙箱 SIGKILL，沿用 3.9.10-3.9.13 经验）：
...
本阶段**允许**复用 `agents/external_staging_provisioning/`（3.9.13）、
`agents/external_staging_runtime/`（3.9.14 新增只读/资格/包聚合层）、
...
# 下一 Phase 编号（本阶段为 3.9.14，禁止提前泄漏 3.9.15）。
NEXT_PHASE_TOKEN = "3.9.15"
...
def main() -> int:
    ...
    if next_hits:
        errors.append(
            "[next-phase-leak] git 视图出现 3.9.15 路径残留（禁止提前进入下一 Phase）：\n  - "
            + "\n  - ".join(next_hits[:20])
        )
    else:
        print("[PASS] 无 3.9.15 路径残留（git 视图）")
