#!/usr/bin/env python3
"""Phase 3.9.13 Branch Integrity Gate.

Fail-closed CI 守卫：在 commit / CI 前核验当前工作树未偏离 Phase 3.9.13
External Staging Provisioning Execution & Resource Registration 的合法边界。

检查项：
  1. 当前分支必须 = feat/phase3.9.13-external-staging-provisioning-execution-registration
  2. 工作树（tracked + untracked）不得出现 forbidden 模块名：
     production_handoff / handoff
  3. 不得出现下一 Phase 编号泄漏（3.9.14）
  4. 旧 WIP carryover：agents/enterprise/production_handoff/ 不得出现
  5. Audit Ledger total 必须 = 129（本阶段 0 新增企业类目；3.9.13 执行审计类别为
     自包含常量集，不污染企业 AuditActionCategory 枚举与冻结账本）

设计约束（规避沙箱 SIGKILL，沿用 3.9.10-3.9.12 经验）：
...
本阶段**允许**复用 `agents/external_staging_provisioning/`（3.9.12）、
`agents/external_staging_qualification/`（3.9.10）、`agents/external_staging_execution/`
（3.9.11）与 `agents/staging_runtime/`，不视为 forbidden；仅禁止 Production Handoff
旧 WIP 与下一 Phase（3.9.14）泄漏。
...
EXPECTED_BRANCH = "feat/phase3.9.13-external-staging-provisioning-execution-registration"
...
# 下一 Phase 编号（本阶段为 3.9.13，禁止提前泄漏 3.9.14）。
NEXT_PHASE_TOKEN = "3.9.14"
...
def main() -> int:
    ...
    if next_hits:
        errors.append(
            "[next-phase-leak] git 视图出现 3.9.14 路径残留（禁止提前进入下一 Phase）：\n  - "
            + "\n  - ".join(next_hits[:20])
        )
    else:
        print("[PASS] 无 3.9.14 路径残留（git 视图）")
