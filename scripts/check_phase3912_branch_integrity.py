#!/usr/bin/env python3
"""Phase 3.9.12 Branch Integrity Gate.

Fail-closed CI 守卫：在 commit / CI 前核验当前工作树未偏离 Phase 3.9.12
External Staging Provisioning Operator Readiness 的合法边界。

检查项：
  1. 当前分支必须 = feat/phase3.9.12-external-staging-provisioning-operator-readiness
  2. 工作树（tracked + untracked）不得出现 forbidden 模块名：
     production_handoff / production_change / handoff
  3. 不得出现下一 Phase 编号泄漏（3.9.13）
  4. 旧 WIP carryover：agents/enterprise/production_handoff/ 不得出现
  5. Audit Ledger total 必须 = 129（本阶段 0 新增企业类目；3.9.12 供给审计类别为
     自包含常量集，不污染企业 AuditActionCategory 枚举与冻结账本）

设计约束（规避沙箱 SIGKILL，沿用 3.9.10/3.9.11 经验）：
...
本阶段**允许**复用 `agents/external_staging_qualification/`（3.9.10）、
`agents/external_staging_execution/`（3.9.11）与 `agents/staging_runtime/`，
不视为 forbidden；仅禁止 Production Handoff 旧 WIP 与下一 Phase（3.9.13）泄漏。
...
# 注意：production_change / production_change_readiness 等为 3.9.7/3.9.9 合法提交的变更管控
# 模块，不属于本阶段守卫范围，不得误标。
FORBIDDEN_PATH_SEGMENTS = ("production_handoff", "handoff")
# 下一 Phase 编号（本阶段为 3.9.12，禁止提前泄漏 3.9.13）。
NEXT_PHASE_TOKEN = "3.9.13"
...
def main() -> int:
    ...
    if next_hits:
        errors.append(
            "[next-phase-leak] git 视图出现 3.9.13 路径残留（禁止提前进入下一 Phase）：\n  - "
            + "\n  - ".join(next_hits[:20])
        )
    else:
        print("[PASS] 无 3.9.13 路径残留（git 视图）")
    ...
    if total is None:
        ...
        errors.append(
            ...
            f"Phase 3.9.12 不得引入新企业审计类目（3.9.12 供给审计类别为自包含常量集）"
        )
    else:
