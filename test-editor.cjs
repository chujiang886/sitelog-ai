...
(async()=>{
 ...
 await page.getByPlaceholder('项目名称',{exact:true}).fill('第一窗口旧版不能覆盖');await page.waitForFunction(()=>Alpine.$data(document.body).draftConflict);assert.ok((await page.locator('[x-text="draftStatus"]').innerText()).includes('另一个窗口'));
