...
if (!base || !/^http:\/\/127\.0\.0\.1:\d+$/.test(base)) throw Error('本测试只允许隔离的本机测试服务器');
...
(async () => {
  ...
  await login(admin, 'admin', 'Test-admin-initial-12345');
  await changePassword(admin, 'Test-admin-initial-12345', 'Test-admin-ready-12345');
  await login(admin, 'admin', 'Test-admin-ready-12345');
  ...
  await admin.getByLabel('新增员工账号').fill('worker01');
  await admin.getByLabel('新增员工初始密码').fill('Test-worker-initial-12345');
  ...
  await login(worker, 'worker01', 'Test-worker-initial-12345');
  await changePassword(worker, 'Test-worker-initial-12345', 'Test-worker-ready-12345');
  await login(worker, 'worker01', 'Test-worker-ready-12345');
  ...
  await login(second, 'worker01', 'Test-worker-ready-12345');
  ...
  assert.ok(cookies.find(c => c.name === 'cj_sitelog_session' && c.httpOnly && c.sameSite === 'Lax'));
